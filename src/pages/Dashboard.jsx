...
const Dashboard = () => {
  ...
  return (
    <div>
      ...
      <div className="mb-6 grid grid-cols-1 gap-3 rounded-lg border border-slate-200 bg-white p-4 sm:grid-cols-2 lg:grid-cols-5">
        <input
          type="text"
      ...
      {folderError && (
        <p className="mb-4 rounded-md bg-red-50 px-3 py-2 text-sm text-red-600">{folderError}</p>
      )}
      {deleteError && (
        <p className="mb-4 rounded-md bg-red-50 px-3 py-2 text-sm text-red-600">{deleteError}</p>
      )}
      ...
      {loading ? (
        ...
        <div className="rounded-lg border border-dashed border-slate-300 py-16 text-center text-sm text-slate-400">
          Thư mục trống. Hãy tạo thư mục hoặc tải lên tài liệu đầu tiên!
        </div>
      ) : documents.length > 0 ? (
